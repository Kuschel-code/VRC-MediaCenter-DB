...
def _proxy_noredirect(url: str) -> str | None:
    """Nutzt den Proxy mit noredirect=1 um die 302 Location zu erfassen."""
    import urllib.parse
    proxy_url = f"{WORKER_PROXY}?url={urllib.parse.quote(url, safe='')}&noredirect=1"
    try:
        resp = httpx.get(proxy_url, timeout=20)
        if resp.status_code == 200:
            import json
            ...
            if redirect:
                log.info(f"[Proxy/NoRedirect] 302 → {redirect[:80]}")
                return redirect
    except Exception as e:
        log.info(f"[Proxy/NoRedirect] Fehler: {e}")
    return None
...
def _resolve_serienstream_hoster(episode_html: str, base_url: str) -> str | None:
    ...
    # Step 1: Finde iframe mit /r?t= Token
    iframe = soup.select_one("iframe[src*='/r?t=']")
    if not iframe:
        log.info("[STO] Kein iframe mit /r?t= gefunden")
        return None
    ...
    # Step 2: Noredirect proxy um 302 Location zu bekommen (z.B. voe.sx/e/xxx)
    hoster_url = _proxy_noredirect(iframe_src)
    if not hoster_url:
